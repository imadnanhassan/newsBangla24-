...
const navigation: MenuItem[] = [
  ...
  },
  {
    ...
    icon: 'FileText',
    children: [
      { id: 'all-articles', label: 'সব নিবন্ধ', href: '/reporter/articles', icon: 'FileText' },
      { id: 'create-article', label: 'নতুন নিবন্ধ', href: '/reporter/articles/create', icon: 'PlusCircle' },
      { id: 'drafts', label: 'খসড়া', href: '/reporter/articles/drafts', icon: 'Edit', badge: '3', badgeColor: 'bg-amber-500' },
      { id: 'pending', label: 'পর্যালোচনায়', href: '/reporter/articles/pending', icon: 'Clock', badge: '2', badgeColor: 'bg-blue-500' },
      { id: 'published', label: 'প্রকাশিত', href: '/reporter/articles/published', icon: 'CheckCircle' },
      { id: 'rejected', label: 'প্রত্যাখ্যাত', href: '/reporter/articles/rejected', icon: 'AlertCircle' },
    ]
  },
  {
    id: 'media',
    ...
    icon: 'Camera',
    children: [
      { id: 'all-media', label: 'সব মিডিয়া', href: '/reporter/media', icon: 'Camera' },
      { id: 'upload', label: 'ছবি আপলোড', href: '/reporter/media/upload', icon: 'Upload' },
      { id: 'videos', label: 'ভিডিও', href: '/reporter/media/videos', icon: 'Video' },
      { id: 'gallery', label: 'গ্যালারি', href: '/reporter/media/gallery', icon: 'Eye' },
    ]
  },
  {
    id: 'analytics',
    ...
    icon: 'BarChart3',
    children: [
      { id: 'analytics-overview', label: 'অ্যানালিটিক্স', href: '/reporter/analytics', icon: 'Activity' },
      { id: 'views', label: 'ভিউ রিপোর্ট', href: '/reporter/analytics/views', icon: 'Eye' },
      { id: 'engagement', label: 'এনগেজমেন্ট', href: '/reporter/analytics/engagement', icon: 'Target' },
    ]
  },
  {
    id: 'calendar',
    label: 'ক্যালেন্ডার',
    href: '/reporter/calendar',
    icon: 'Calendar',
  },
  {
    id: 'bookmarks',
    label: 'বুকমার্ক',
    href: '/reporter/bookmarks',
    icon: 'Bookmark',
  },
  {
    id: 'notifications',
...
const iconMap = {
  Home, FileText, PlusCircle, Camera, Video, BarChart3, Settings, User,
  Calendar, MessageSquare, Clock, CheckCircle, AlertCircle, Upload,
  Eye, Edit, Bookmark, Bell, Search, ChevronDown, TrendingUp, Award,
  Zap, Target, Activity
};
...
export default function ReporterSidebar() {
  ...
  if (!mounted) {
    return (
      <div className="w-72 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 h-full">
  ...
  return (
    <div className="w-72 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 shadow-2xl border-r border-slate-700 h-full flex flex-col overflow-hidden">
      {/* Logo Section */}
      <div className="p-6 border-b border-slate-700/50">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">প্রফেশনাল নিউজ সেন্টার</p>
          </div>
        </div>
      </div>
                ...
                <div>
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 group ${
                      isActive(item.href!)
                        ? 'bg-gradient-to-r from-red-500/20 to-red-600/20 text-red-300 border border-red-500/30 shadow-lg shadow-red-500/10'
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2">
                      {item.badge && (
                        <span className={`${item.badgeColor || 'bg-red-500'} text-white text-xs px-2 py-1 rounded-full font-semibold shadow-lg`}>
                          {item.badge}
                        </span>
                      )}
                      <div className={`transition-transform duration-200 ${isExpanded(item.id) ? 'rotate-180' : ''}`}>
                        ...
                        return (
                          <Link
                            ...
                            className={`flex items-center justify-between px-4 py-2.5 text-sm rounded-lg transition-all duration-200 group ${
                              isActive(child.href!)
                                ? 'bg-gradient-to-r from-red-500/15 to-red-600/15 text-red-300 border-l-2 border-red-500'
                          ...
                          >
                            ...
                            {child.badge && (
                              <span className={`${child.badgeColor || 'bg-slate-600'} text-white text-xs px-2 py-0.5 rounded-full font-semibold`}>
                                {child.badge}
                              </span>
                            )}
                          </Link>
      ...
      <div className="flex-shrink-0 p-4 border-t border-slate-700/50">
        <div className="bg-gradient-to-br from-slate-800/80 to-slate-700/80 backdrop-blur-sm rounded-xl p-4 border border-slate-600/30">
          <div className="flex items-center space-x-2 mb-3">
            ...
            <h4 className="text-sm font-semibold text-white">আজকের পরিসংখ্যান</h4>
          </div>
          <div className="space-y-3">
          ...
          <div className="mt-4 pt-3 border-t border-slate-600/30">
            <div className="flex items-center space-x-2 text-xs">
              ...
              <span className="text-slate-300 font-medium">এই সপ্তাহের সেরা রিপোর্টার</span>
            </div>
          </div>
        </div>
      </div>
